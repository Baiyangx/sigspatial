...
%     \KwIn{$\X, \Y$}
%     \KwOut{$\W$}
%     Initialize $\rho = 1$, $\W^{(0)}$, $\U_{w}^{(0)}$, $\U_{1}^{(0)}$\;
%     Initialize $\epsilon^{r} > 0, \epsilon^{s} > 0$, $\mathrm{MAX\_ITER}$\;
%     \For{$k = 1 : \mathrm{MAX\_ITER}$}
%     {%
%         %\tcp{Update primal variables.}
%         Update $\W^{(k)}$ with BCD using~\ref{opt:grad}\;
%         %\tcp{Update dual variables.}
%         Update $\U_{w}^{({k})}$ with $\prox_{f_{1}, 1/\rho}(\U_{1}^{(k - 1)}
%         + \W^{(k)})$\;
%         %\tcp{Update Lagrangian multiplier.}
%         Update $\U_{1}^{({k})}$ with Equation~\ref{equ:update_lm}\;
%         Compute $r$ and $s$ by Equations~\ref{equ:update_residuals}\;
%         \If{$r < \epsilon^{r}$ and $s < \epsilon^{s}$}
%         {%
%             break\;
%         }
%         %\If{$r > 10s$}
...
%         %}
%     }
%     \caption{LDA-BERT Topical Clustering}\label{alg:admm}
% \end{algorithm}
\begin{figure}[h]
\centering
