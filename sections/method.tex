...
From a holistic view of research method (Figure~\ref{fig:Research Method}), it consists of three
...
database publisher Figure~\ref{fig:Tweet dataset}. We use tweets ID to request twitter API in batch
...
removal of stop words (Figure~\ref{fig:Tokenization and tag}). Regex filtering process used to delete some
...
Example of these regular expression technique is shown in the (Figure~\ref{fig:Shows the conceptual}): 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{imgs/cleaning_w_regex.png}
    \caption{\label{fig:Right-most column}Right-most column showing cleaned text with Regex}
    \label{fig:regex}
\end{figure}
...
\caption{\label{fig:Shows the conceptual} Shows the conceptual document space and topic clusters}
\end{figure}
The size of the circle in (Figure~\ref{fig:Shows the conceptual2}) determines the number of documents that
...
\end{figure}
In the (Figure~\ref{fig:Completed dataset}), the circle size of the term clusters visualize the frequency
